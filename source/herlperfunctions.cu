...
std::tuple<double, double> minmax( cuDoubleComplex* buffer, int size, bool device_pointer ) {
    if ( device_pointer ) {
        thrust::device_ptr<cuDoubleComplex> dev_buffer = thrust::device_pointer_cast( buffer );
        auto mm = thrust::minmax_element( thrust::device, dev_buffer, dev_buffer + size, compare_complex_abs2() );
        cuDoubleComplex min = *mm.first;
        cuDoubleComplex max = *mm.second;
...
std::tuple<double, double> minmax( double* buffer, int size, bool device_pointer ) {
    if (device_pointer) {
        thrust::device_ptr<double> dev_buffer = thrust::device_pointer_cast(buffer);
        auto mm = thrust::minmax_element( thrust::device, dev_buffer, dev_buffer + size, thrust::less<double>() );
        double min = *mm.first;
        double max = *mm.second;
        return std::make_tuple( min, max );
    }
    auto mm = thrust::minmax_element( buffer, buffer + size, thrust::less<double>() );
