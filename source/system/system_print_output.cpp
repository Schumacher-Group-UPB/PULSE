...
void PC3::System::printHelp() {
...
#endif
    std::string seperator = std::string(160, '-');
    std::cout
        << EscapeSequence::BOLD << seperator << EscapeSequence::RESET << std::endl
    ...
    std::cout << unifyLength( "Envelopes.", "", "" ) << std::endl
              << "Envelopes are passed using either their spatial and temporal characteristics, or by loading an external file. Syntax:" << std::endl
