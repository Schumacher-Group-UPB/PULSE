    ...
    for ( int col = 0; col < s_N; col++ ) {
        for ( int row = 0; row < s_N; row++ ) {
            ...
            for ( int c = 0; c < mask.amp.size(); c++ ) {
                ...
                complex_number charge = pow( complex_number( ( x - mask.x[c] ) / xmax, 1.0 * sign( mask.m[c] ) * ( y - mask.y[c] ) / xmax ), abs( mask.m[c] ) );
