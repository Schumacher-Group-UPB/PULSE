...
PC3::Envelope PC3::Envelope::fromCommandlineArguments( int argc, char** argv, const std::string& key, const bool time ) {
    ...
    while ( ( index = findInArgv( "--" + key, argc, argv, index ) ) != -1 ) {
        ...
        ret.addSpacial( amp, width, posX, posY, exponent, stype, spol, sbehavior );

        if ( not time )
            continue;

        // Temporal Component
        real_number t0 = getNextInput( argv, "pulse.t0", index );
        real_number freq = getNextInput( argv, "pulse.freq", index );
        real_number sigma = getNextInput( argv, "pulse.sigma", index );
        ret.addTemporal( t0, sigma, freq );
        // Charge
        int m = getNextInput( argv, "pulse.m", index );
