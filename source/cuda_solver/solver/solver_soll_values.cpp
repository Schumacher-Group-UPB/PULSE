...
void PC3::Solver::calculateSollValues() {
    ...
    if ( system.normalize_before_masking ) {
        std::tie( min_mask_plus, max_mask_plus ) = CUDA::minmax( host.soll_plus.get(), system.s_N_x * system.s_N_y, false /*Device Pointer*/ );
        std::tie( min_psi_plus, max_psi_plus ) = CUDA::minmax( host.wavefunction_plus.get(), system.s_N_x * system.s_N_y, false /*Device Pointer*/ );
        ...
        if (system.use_te_tm_splitting) {
            std::tie( min_mask_minus, max_mask_minus ) = CUDA::minmax( host.soll_minus.get(), system.s_N_x * system.s_N_y, false /*Device Pointer*/ );
            std::tie( min_psi_minus, max_psi_minus ) = CUDA::minmax( host.wavefunction_minus.get(), system.s_N_x * system.s_N_y, false /*Device Pointer*/ );
        ...
        // Devide error by N^2
        max_mask_plus *= system.s_N_x * system.s_N_y;
        max_psi_plus *= system.s_N_x * system.s_N_y;
        max_mask_minus *= system.s_N_x * system.s_N_y;
        max_psi_minus *= system.s_N_x * system.s_N_y;
    }
...
#pragma omp parallel for
    ...
    std::ranges::for_each( host.soll_plus.get(), host.soll_plus.get() + system.s_N_x * system.s_N_y, [&sum_plus]( real_number n ) { sum_plus += n; } );
    ...
    #pragma omp parallel for
        ...
        std::ranges::for_each( host.soll_minus.get(), host.soll_minus.get() + system.s_N_x * system.s_N_y, [&sum_minus]( real_number n ) { sum_minus += n; } );
