...
#pragma omp parallel for
        for ( int i = 0; i < fileoutputkeys.size(); i++ ) {
            ...
            if ( system.fft_every < system.t_max and key == "fft_plus" and system.doOutput( "fft_mask", "fft", "fft_plus", "plus", "mat", "all" ) ) {
                ...
                filehandler.outputMatrixToFile( buffer.data(), start_x, end_x, start_y, end_y, system.p.N_x, system.p.N_y, increment, fft_header_information, prefix + key + suffix );
            }
...
void PC3::Solver::outputInitialMatrices() {
    ...
    if ( system.doOutput( "all", "mat", "initial_plus", "initial" ) ) {
        system.filehandler.outputMatrixToFile( matrix.initial_state_plus.data(), system.p.N_x, system.p.N_y, header_information, "initial_wavefunction_plus" );
        system.filehandler.outputMatrixToFile( matrix.initial_reservoir_plus.data(), system.p.N_x, system.p.N_y, header_information, "initial_reservoir_plus" );
    }
    ...
    if ( system.doOutput( "all", "mat", "initial_minus", "initial" ) ) {
        system.filehandler.outputMatrixToFile( matrix.initial_state_minus.data(), system.p.N_x, system.p.N_y, header_information, "initial_wavefunctions_minus" );
        system.filehandler.outputMatrixToFile( matrix.initial_reservoir_minus.data(), system.p.N_x, system.p.N_y, header_information, "initial_reservoir_minus" );
    }    
