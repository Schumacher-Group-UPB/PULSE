...
void timeitStatisticsSummary( System& s, FileHandler& handler ) {
    ...
    for (int i = 0; i < s.pump_amp.size(); i++) {
        std::cout << "Pump at amp = " << (s.pump_amp[i] == -666 ? " adaptive" : std::to_string(s.pump_amp[i])) << ", width = " << s.pump_width[i] << ", X = " << s.pump_X[i] << ", Y = " << s.pump_Y[i] << ", pol = " << s.pump_pol[i] << ", type = " << s.pump_type[i] << std::endl;
    }
