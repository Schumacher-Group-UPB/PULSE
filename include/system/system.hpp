...
class System {
   public:
    ...
    struct Parameters {
        ...
        real_number hbar_s, one_over_h_bar_s;
        ...
        Parameters( unsigned int N, real_number t, real_number dt, real_number xmax, real_number dx, 
            real_number m_e, real_number h_bar_s, real_number m_eff, real_number gamma_c, real_number gamma_r, real_number g_c, real_number g_r, real_number R, real_number g_pm, real_number delta_LT ) : 
        N(N), N2(N*N), t(t), dt(dt), dt_half(dt/real_number(2.0)), xmax(xmax), dx(dx), 
        m_e(m_e), h_bar_s(h_bar_s), m_eff(m_eff), gamma_c(gamma_c), gamma_r(gamma_r), g_c(g_c), g_r(g_r), R(R), g_pm(g_pm), delta_LT(delta_LT), 
        hbar_s(h_bar_s), one_over_h_bar_s(real_number(1.0)/h_bar_s), minus_i_over_h_bar_s( complex_number( 0.0, real_number(-1.0)/h_bar_s ) ), i_h_bar_s( complex_number( 0.0, h_bar_s ) ), 
        half_i( complex_number( 0.0, 0.5 ) ), i( complex_number( 0.0, 1.0 ) ), minus_half_i( complex_number( 0.0, real_number(-0.5) ) ), minus_i( complex_number( 0.0, real_number(-1.0) ) )
        {    
    ...
    // SI Rescaling Units
    real_number m_e, h_bar, e_e, h_bar_s;
    ...
    bool fixed_time_step, normalize_before_masking, randomly_initialize_system, use_te_tm_splitting;
    ...
    */
    ...
    Parameters getKernelParameters() {
        return Parameters( s_N, t, dt, xmax, dx, m_e, h_bar_s, m_eff, gamma_c, gamma_r, g_c, g_r, R, g_pm, delta_LT);
    }
