...
#define INTERMEDIATE_SUM_K(index, ...)                                                                                                                                                             \
    {                                                                                                                                                                                              \
        ...
        Type::complex *k_vec_wf_plus = matrix.k_wavefunction_plus.getDevicePtr(subgrid, index - 1);                                                                                                \
        CALL_SUBGRID_KERNEL(Kernel::Summation::runge_sum_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, true, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,         \
                            current_halo, current_time, kernel_arguments, io.in_wf_plus, io.out_wf_plus, k_vec_wf_plus);                                                                     \
        if (system.p.use_reservoir)                                                                                                                                                                \
        {                                                                                                                                                                                          \
            Type::complex *k_vec_res_plus = matrix.k_reservoir_plus.getDevicePtr(subgrid, index - 1);                                                                                              \
            CALL_SUBGRID_KERNEL(Kernel::Summation::runge_sum_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, false, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,     \
                                current_halo, current_time, kernel_arguments, io.in_rv_plus, io.out_rv_plus, k_vec_res_plus);                                                              \
        }                                                                                                                                                                                          \
        if (system.p.use_twin_mode)                                                                                                                                                                \
        {                                                                                                                                                                                          \
            Type::complex *k_vec_wf_minus = matrix.k_wavefunction_minus.getDevicePtr(subgrid, index - 1);                                                                                          \
            CALL_SUBGRID_KERNEL(Kernel::Summation::runge_sum_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, true, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,     \
                                current_halo, current_time, kernel_arguments, io.in_wf_minus, io.out_wf_minus, k_vec_wf_minus);                                                              \
            if (system.p.use_reservoir)                                                                                                                                                            \
            {                                                                                                                                                                                      \
                Type::complex *k_vec_res_minus = matrix.k_reservoir_minus.getDevicePtr(subgrid, index - 1);                                                                                        \
                CALL_SUBGRID_KERNEL(Kernel::Summation::runge_sum_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, false, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream, \
                                    current_halo, current_time, kernel_arguments, io.in_rv_minus, io.out_rv_minus, k_vec_res_minus);                                                       \
            }                                                                                                                                                                                      \
        }                                                                                                                                                                                          \
    };
...
#define FINAL_SUM_K(index, ...)                                                                                                                                                                           \
    {                                                                                                                                                                                              \
        ...
        CALL_SUBGRID_KERNEL(Kernel::Summation::runge_add_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, true, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,         \
                            0, current_time, kernel_arguments, io.out_wf_plus, k_vec_wf_plus);                                                                     \
        if (system.p.use_reservoir)                                                                                                                                                                \
        {                                                                                                                                                                                          \
            Type::complex *k_vec_res_plus = matrix.k_reservoir_plus.getDevicePtr(subgrid);                                                                                              \
            CALL_SUBGRID_KERNEL(Kernel::Summation::runge_add_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, false, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,     \
                                0, current_time, kernel_arguments, io.out_rv_plus, k_vec_res_plus);                                                              \
        }                                                                                                                                                                                          \
        if (system.p.use_twin_mode)                                                                                                                                                                \
        {                                                                                                                                                                                          \
            Type::complex *k_vec_wf_minus = matrix.k_wavefunction_minus.getDevicePtr(subgrid);                                                                                          \
            CALL_SUBGRID_KERNEL(Kernel::Summation::runge_add_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, true, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream,     \
                                0, current_time, kernel_arguments, io.out_wf_minus, k_vec_wf_minus);                                                              \
            if (system.p.use_reservoir)                                                                                                                                                            \
            {                                                                                                                                                                                      \
                Type::complex *k_vec_res_minus = matrix.k_reservoir_minus.getDevicePtr(subgrid);                                                                                        \
                CALL_SUBGRID_KERNEL(Kernel::Summation::runge_add_to_input_k<GCC_EXPAND_VA_ARGS_ORDER(Type::complex, false, index, __VA_ARGS__)>, "Sum for K" #index, current_grid, current_block, stream, \
                                    0, current_time, kernel_arguments, io.out_rv_minus, k_vec_res_minus);                                                       \
            }                                                                                                                                                                                      \
        }                                                                                                                                                                                          \
    };
...
#define CALL_SUBGRID_KERNEL(func, name, grid, block, stream, ...)              \
    {                                                                          \
        ...
        _Pragma("omp simd") for (Type::uint32 i = 0; i < execution_range; ++i) \
        {                                                                      \
            func(i, __VA_ARGS__);                                              \
        }                                                                      \
    }
