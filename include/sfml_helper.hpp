...
void plotMatrix( BasicWindow& window, T* buffer, int N, int posX, int posY, int skip, ColorPalette& cp, const std::string& title = "" ) {
    ...
    auto [min, max] = normalize( plotarray.get(), N * N );
...
void initSFMLWindow( System& system, MatrixHandler& matrixhandler ) {
#ifdef SFML_RENDER
    if ( matrixhandler.disableRender ) {
    ...
    if ( matrixhandler.colorPalette.find( ".pal" ) != std::string::npos )
        colorpalette.readColorPaletteFromGnuplotDOTPAL( matrixhandler.colorPalette );
    else
        colorpalette.readColorPaletteFromTXT( matrixhandler.colorPalette );
...
bool plotSFMLWindow( System& system, MatrixHandler& handler, Buffer& buffer ) {
#ifdef SFML_RENDER
